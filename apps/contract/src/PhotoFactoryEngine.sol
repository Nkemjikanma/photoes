  ...
  IPhotoFactoryEngine
{
  ...
  address[] public deployedPhotoFactoryContracts;
  ...
  modifier copiesOwnedToAiCheck(address _imageOwner, uint256 _tokenId) {
    EditionOwnership storage ownership = editionOwnership[_tokenId][
      _imageOwner
    ];
  ...
  function mint(
    ...
    if (_editionSize == 1) {
      ...
      try new PhotoFactory721(_photoName, msg.sender) returns (
        PhotoFactory721 newFactory721
      ) {
      ...
      try new PhotoFactory1155(_photoName, msg.sender) returns (
        PhotoFactory1155 newFactory1155
      ) {
  ...
  function purchase(
    ...
    (bool isSingleEdition, bool isMultipleEdition) = decidePhotoEdition(
      _tokenId
    );
  ...
  function _purchaseSingleEdition(
    PhotoItem storage _photo,
    uint256 _payment,
    uint256 _tokenId
  ) private {
    ...
    editionOwnership[_tokenId][msg.sender] = EditionOwnership({
      copiesOwned: 1,
      aiVariantIds: new uint256[](0),
      canMintAi: true
    });
    ...
    nonReentrant
  {
    ...
    try factory721.mintERC721(aiTokenURI, aiVariant.variantId) {
      ...
      if (isMultipleEdition) {
        multiplePhotoItems[_tokenId].aiVariantTokenIds.push(
          aiVariant.variantId
        );
      }
  ...
  function verifyMint(uint256 tokenId) public view returns (bool) {
    bool minted = false;
    if (
      s_photoItem[tokenId].minted == true || multiplePhotoItems[tokenId].minted
    ) {
      minted = true;
    }
  ...
  function getMultiplePhotoItems(
    uint256 _tokenId
  ) public view returns (MultiplePhotoItems memory) {
