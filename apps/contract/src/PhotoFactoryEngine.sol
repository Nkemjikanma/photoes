  ...
  IERC721Receiver
{
  ...
  function generateAiVariant(
    uint256 _tokenId,
    string memory _tokenURI
  ) internal nonReentrant onlyMinted(_tokenId) returns (bytes32 requestId) {
  ...
  function mintAiVariant(
    uint256 _tokenId
  ) public onlyPhotoOwner(_tokenId) onlyMinted(_tokenId) nonReentrant {
